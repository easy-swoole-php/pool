...
abstract class AbstractPool
{
    ...
    /**
     * @param float|null $timeout  获取池对象超时时间
     * @param int        $tryTimes 出现异常重试次数
     *
     * @return ObjectInterface|mixed|null
     * @throws Exception
     * @throws Throwable
     */
    ...
    {
        /**
         * 懒惰模式，可以提前创建 pool 对象，因此调用前执行初始化检测
         */
        $this->init();
    ...
    public function reset(): AbstractPool
    {
        $this->destroy();
        $this->createdNum = 0;
        $this->destroy    = false;
    ...
    public function recycleObjByCid(int $cid)
    {
        if (isset($this->context[$cid])) {
            $obj = $this->context[$cid];
            unset($this->context[$cid]);
